    ...
    heart.addEventListener("click", () => {
        ...
        if (score === 10) {
            ...
            const countdownInterval = setInterval(() => {
                countdownMessage.textContent = `Bieeen! 💙\n espera ${countdown} segundos...`;
                countdown--;
...
function createParticle() {
    return {
        ...
        emoji: emojis[Math.floor(Math.random() * emojis.length)], // Selección aleatoria de emojis
    };
}
...
function drawParticles() {
    ...
    for (let particle of particles) {
        ...
        ctx.fillText(particle.emoji, particle.x, particle.y); // Dibuja el emoji seleccionado
        particle.y -= particle.speedY;

        if (particle.y < -10) {
            particle.y = canvas.height;
            particle.x = Math.random() * canvas.width;
            particle.emoji = emojis[Math.floor(Math.random() * emojis.length)]; // Cambiar el emoji al reiniciar
        }
    }
